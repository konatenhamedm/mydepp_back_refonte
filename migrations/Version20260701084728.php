 ...
 *
 * NB: quasi-doublon de Version20260701072404 (régénéré via doctrine:migrations:diff),
 * toutes les étapes sont donc gardées par des vérifications d'existence. La seule
 * différence réelle est la colonne `etape` sur autre_document_professionnel, ajoutée
 * explicitement à la fin pour converger vers le même schéma quel que soit l'ordre
 * d'exécution sur une base donnée.
 */
final class Version20260701084728 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        } elseif (!$this->columnExists('autre_document_professionnel', 'etape')) {
            $this->addSql('ALTER TABLE autre_document_professionnel ADD etape VARCHAR(255) DEFAULT NULL');
        }

        if (!$this->tableExists('presence')) {
        ...
        if (!$this->tableExists('type_autre_document')) {
        ...
        if ($this->constraintExists('membre_professionnel', 'FK_5D832F3D9732555')) {
            $this->addSql('ALTER TABLE membre_professionnel DROP FOREIGN KEY FK_5D832F3D9732555');
        }
        if ($this->indexExists('membre_professionnel', 'IDX_5D832F3D9732555')) {
        ...
        if ($this->columnExists('membre_professionnel', 'lieu_obtention_diplome_id') && !$this->columnExists('membre_professionnel', 'origine_diplome_id')) {
            $this->addSql('ALTER TABLE membre_professionnel CHANGE lieu_obtention_diplome_id origine_diplome_id INT DEFAULT NULL');
        }
        if ($this->columnExists('membre_professionnel', 'origine_diplome_id') && !$this->constraintExists('membre_professionnel', 'FK_5D832F3DB3799188')) {
        ...
        if ($this->columnExists('membre_professionnel', 'origine_diplome_id') && !$this->indexExists('membre_professionnel', 'IDX_5D832F3DB3799188')) {
